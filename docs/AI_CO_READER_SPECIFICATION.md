...
| | officeparser / mammoth | PPTX text extraction |
| | Tesseract.js / OCR.space API | OCR for scanned documents |
...
// Supports: PDF, PPTX, and scanned documents (with OCR)
async function uploadDocument(file: File) {
  ...
  if (fileType === 'application/pdf') {
    ...
    if (isScanned) {
      ...
      extractedText = await extractTextWithOCR(file, 'pdf');
    } else {
    ...
    // Direct OCR for image files (scanned documents)
    extractedText = await extractTextWithOCR(file, 'image');
  }
  ...
  await vectorDB.upsert({
    ...
    vectors: chunks.map((chunk, idx) => ({
      ...
      metadata: {
        ...
        slideNumber: chunk.slideNumber, // For PPTX
        title: file.name,
        uploadedBy: userId,
        fileType: fileType
      }
  ...
  return { 
    documentId: docId, 
    status: 'indexed',
    ocrProcessed: isScanned || fileType.startsWith('image/')
  };
}
```
...
file: [binary file data - PDF, PPTX, or scanned document]
courseId: "CHM101"
title: "Module 1: Atomic Theory"
enableOCR: true  // Optional: force OCR processing
```
...
```json
{
  ...
  "document": {
    ...
    "chunkCount": 156,
    "ocrProcessed": false,
    "supportedFormats": ["pdf", "pptx", "jpg", "png"]
  }
}
```
...
└── services/
    ...
    ├── pptxProcessor.js          # PowerPoint text extraction
    ├── ocrService.js             # OCR for scanned documents
...
```typescript
{
  ...
  title: String,
  filename: String,
  fileType: String, // 'application/pdf', 'application/vnd...pptx', 'image/jpeg', etc.
  uploadedBy: ObjectId,
  courseId: String,
  fileUrl: String, // S3 URL or GridFS file ID
  pages: Number,
  slides: Number, // For PPTX files
  chunkCount: Number,
  indexStatus: 'pending' | 'processing' | 'completed' | 'failed',
  ocrProcessed: Boolean,
  ocrConfidence: Number, // Average confidence score for OCR
  metadata: {
    fileSize: Number,
    contentType: String,
    uploadedAt: Date,
    language: String // Detected language from OCR
  },
